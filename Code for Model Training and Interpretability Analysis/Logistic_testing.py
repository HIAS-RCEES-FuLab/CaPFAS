...
X_train = np.vstack([dataset_nonstandard[i][0] for i in range(len(dataset_nonstandard))])
y_train = np.hstack([dataset_nonstandard[i][1] for i in range(len(dataset_nonstandard))])
...
X_test = np.vstack([test_dataset[i][0] for i in range(len(test_dataset))])
y_test = np.hstack([test_dataset[i][1] for i in range(len(test_dataset))])
files = [test_dataset[i][2] for i in range(len(test_dataset))]
msms_lens = [test_dataset[i][3] for i in range(len(test_dataset))]
losses_list = [test_dataset[i][4] for i in range(len(test_dataset))]
...
for i in range(len(X_test)):
    ...
    records.append({
        ...
        "pred_label": int(y_pred_all[i]),
        "has_cf2": cf2_flag,
        "msms_len": msms_lens[i]
    })
