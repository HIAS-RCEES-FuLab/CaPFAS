...
from sklearn.svm import LinearSVC  # 使用线性核的 SVM
...
train_acc = accuracy_score(y_train, svm_clf.predict(X_train))
test_acc = accuracy_score(y_test, svm_clf.predict(X_test))
print(f"Linear SVM - Train Acc: {train_acc:.4f} - Test Acc: {test_acc:.4f}")

save_path = "linear_svm_model.pkl"
joblib.dump(svm_clf, save_path)
print(f"✅ Linear SVM model saved to {save_path}")
